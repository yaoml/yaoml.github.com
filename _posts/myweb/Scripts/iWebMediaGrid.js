...
var IWFeed = Class.create({initialize: function (url) {
    ...
    new Ajax.Request(url, {method: 'get', onSuccess: this.p_onload.bind(this, baseURL), onFailure: this.p_requestFailed.bind(this, baseURL)});
...
var IWStreamEntry = Class.create({initialize: function (thumbnailURL, title, richTitle, guid) {
...
var IWImageStreamEntry = Class.create(IWCommentableStreamEntry, {initialize: function ($super, assetURL, showCommentIndicator, imageURL, thumbnailURL, microURL, mipThumbnailURL, title, richTitle, guid) {
...
var IWMovieStreamEntry = Class.create(IWCommentableStreamEntry, {initialize: function ($super, assetURL, showCommentIndicator, movieURL, thumbnailURL, title, richTitle, movieParams, guid) {
    $super(assetURL, showCommentIndicator, thumbnailURL, title, richTitle, guid);
...
var IWMediaStreamPageEntry = Class.create(IWStreamEntry, {initialize: function ($super, targetPageURL, thumbnailURL, title, richTitle, guid) {
    if (arguments.length > 0) {
        $super(thumbnailURL, title, richTitle, guid);
...
function IWMediaStreamPageEntryPositionedThumbnailMarkupForRect(thumbnail, rect) {
    ...
    var scale = Math.max(rect.size.width / thumbnailSize.width, rect.size.height / thumbnailSize.height);
    var imageSize = thumbnailSize.scale(scale, scale, true);
    var imagePosition = new IWPoint((rect.size.width - imageSize.width) / 2, (rect.size.height - imageSize.height) / 2);
...
var IWMediaStreamPhotoPageEntry = Class.create(IWMediaStreamPageEntry, {initialize: function ($super, streamScriptURL, targetPageURL, title, contentsFunction, guid) {
    ...
    loadingArea.innerHTML = '<iframe id=' + uniqueId + ' src="streamloader.html?scriptURL=' + this.mStreamScriptURL + '&id=' + this.mSlideshowId + '" style="position: absolute; visibility: hidden; ">' + '</iframe>';
    ...
    if (this.mThumbnail) {
        if (isiPhone == false) {
            ...
            prefs["mediaStreamObject"] = {load: function (media, baseURL, callback) {
                callback(media);
            }.bind(null, this.mMedia)};
            new SlideshowGlue(this.mSlideshowId, '../Scripts/Widgets/Slideshow', '../Scripts/Widgets/SharedResources', '..', prefs);
        }
    }
}});
...
var IWMediaStreamMediaPageEntry = Class.create(IWMediaStreamPageEntry, {initialize: function ($super, targetPageURL, thumbnailURL, title, contents, isMovie, guid) {
    ...
    for (var i = 0; i < items.length; ++i) {
        ...
        if (link != null) {
            ...
            if (skip == false) {
                ...
                var thumbnail = this.p_firstElementByTagNameNS(item, thumbnailNamespace, 'thumbnail') || item.getElementsByTagName('thumbnail')[0];
                if (thumbnail) {
...
function setFrameOptionallyMovingContents(element, frame, moveContents) {
    ...
    $(element).setStyle({left: px(frame.origin.x), top: px(frame.origin.y), width: px(frame.size.width), height: px(frame.size.height)});
...
function offsetChildren(element, offset, reverse) {
    for (var node = element.firstChild; node; node = node.nextSibling) {
        ...
        $(node).setStyle({left: px(reverse ? left - offset.x : left + offset.x), top: px(reverse ? top - offset.y : top + offset.y)});
    }
}
...
var PhotoView = Class.create({initialize: function (container, scaleMode, reflectionHeight, reflectionOffset, backgroundColor, movieMode, captionHeight) {
    ...
    $(this.div).setStyle({position: "absolute", width: px(box_size.width), height: px(box_size.height), backgroundColor: this.backgroundColor});
    if (this.canvas) {
    ...
    if (this.caption) {
        $(this.caption).setStyle({position: "absolute", top: px(box_size.height - this.captionHeight * 0.6), left: 0, width: px(box_size.width)});
    ...
    if ((img_aspect > box_aspect && this.scaleMode == "fit") || (img_aspect < box_aspect && this.scaleMode == "fill")) {
    ...
    if (this.object) {
        ...
        $(this.object).setStyle({position: "absolute", left: px(this.movieRect.origin.x), top: px(this.movieRect.origin.y)});
    }
...
var SimpleAnimation = Class.create({initialize: function (oncomplete) {
    this.duration = 500;
    this.from = 0.0;
    this.to = 1.0;
    if (oncomplete != null) {
...
var TransitionEffect = Class.create({initialize: function (current, next, oncomplete, shouldTighten) {
    ...
    if (this.shouldTighten && (vert || horiz)) {
        ...
        this.current.originalFrame = new IWRect(this.current.div.offsetLeft, this.current.div.offsetTop, this.current.div.offsetWidth, this.current.div.offsetHeight);
        this.next.originalFrame = new IWRect(this.next.div.offsetLeft, this.next.div.offsetTop, this.next.div.offsetWidth, this.next.div.offsetHeight);
...
var Slideshow = Class.create({initialize: function (container, photos, onchange, options) {
    this.reflectionHeight = 0;
    this.reflectionOffset = 0;
    this.backgroundColor = 'black';
    this.scaleMode = "fit";
    ...
    if (this.advanceAnchor) {
        ...
        $(this.advanceAnchor).setStyle({position: "absolute", width: px(box_size.width), height: px(box_size.height)});
        if (windowsInternetExplorer) {
    ...
    this.transition = new TransitionEffect(current.view, next.view, function () {
...
function IWStartSlideshow(url, imageStream, index, fullScreen) {
    ...
    var slideWindow = window.open(url, 'slideshow', 'scrollbars=no,titlebar=no,location=no,status=no,toolbar=no,resizable=no,width=' + windowWidth + ',height=' + windowHeight + ',top=' + windowTop + ',left=' + windowLeft);
...
function IWSetSpacerHeight(photogrid, height) {
    ...
    var spacerHeight = Math.max(parseInt(photogrid.style.top) + height - spacer.offsetTop, initialSpacerHeight);
...
function IWLayoutPhotoGrid(divID, layout, stroke, imageStream, range, shadow, valignClass, opacity, slideshowParameters, slideshowURL, headerViewID, detailViewID, footerViewID, layoutOptions) {
    ...
    if (detailViewID != null) {
        ...
        if (!detailView.preferenceForKey("mediaStreamObject")) {
            ...
            var mediaStreamObject = {load: function (baseURL, callback) {
    ...
    var renderThumb = function (i, streamIndex) {
        ...
        if (Math.abs(scaledImageSize.width - thumbnailSize.width) <= 2 && Math.abs(scaledImageSize.height - thumbnailSize.height) <= 2) {
        ...
        if (detailViewID) {
            ...
            anchor.setStyle({position: "absolute", left: framed.style.left, top: framed.style.top, width: framed.style.width, height: framed.style.height, zIndex: 1});
        ...
        if (shadow) {
            if (windowsInternetExplorer) {
                ...
                framed.setStyle({left: px(parseFloat(framed.style.left || 0) + left), top: px(parseFloat(framed.style.top || 0) + top), width: px(right - left), height: px(bottom - top)});
                ...
                shadowDiv.style.filter = "progid:DXImageTransform.Microsoft.MaskFilter()" + " progid:DXImageTransform.Microsoft.MaskFilter(color=" + shadow.mColor + ")" + " progid:DXImageTransform.Microsoft.Blur(pixelradius=" + blurRadius + ")" + " progid:DXImageTransform.Microsoft.Alpha(opacity=" + shadow.mOpacity * opacity * 100 + ")";
...
var IWPhotoGridLayout = Class.create({initialize: function (columnCount, maxImageSize, textBoxSize, tileSize, topPadding, bottomPadding, spacing, framePadding) {
    ...
    var translation = new IWPoint(tileSize.width * (index % columnCount) + IWPhotoGridLayoutConstants.kBorderThickness, tileSize.height * Math.floor(index / columnCount) + IWPhotoGridLayoutConstants.kBorderThickness + this.mTopPadding + IWPhotoGridLayoutConstants.vTopSpacing);
    return translation;
}, translationForImageOfSizeAtIndexWithOffset: function (imageSize, index, offset) {
    var tileSize = this.mTileSize;
    var translation = new IWPoint(0, 0);
    var imageAreaSize = new IWSize(tileSize.width, tileSize.height - (IWPhotoGridLayoutConstants.vTextOffsetFromImage + this.mTextBoxSize.height + IWPhotoGridLayoutConstants.vTextOffsetFromBottom));
    ...
    var translation = new IWPoint((tileSize.width - textBoxSize.width) / 2.0, (tileSize.height - textBoxSize.height) - IWPhotoGridLayoutConstants.vTextOffsetFromBottom);
    if (offset != null) {
        translation.x += offset.translation.width;
        translation.y += offset.translation.height;
    }
    ...
    return Math.min((maxImageSize.width - this.mFramePadding.width) / imageSize.width, (maxImageSize.height - this.mFramePadding.height) / imageSize.height);
}});
var IWJobQueue = Class.create({initialize: function () {
    this.mJobQueue = [];
    this.mTimeout = null;
}, addJob: function (job) {
...
var AppleAnimator = Class.create({initialize: function (duration, interval, optionalFrom, optionalTo, optionalCallback) {
    this.startTime = 0;
...
var AppleAnimation = Class.create({initialize: function (from, to, callback) {
    this.from = from;
    this.to = to;
...
function IWCommentSummaryInfoForURL(resourceURL, callback) {
    ...
    new Ajax.Request(summaryURL, {method: 'get', onSuccess: handleSummaryData, onFailure: callback.bind(null, {})});
}
