...
var HeaderControls = Class.create(Widget, {widgetIdentifier: "com-apple-iweb-widget-headercontrols", initialize: function ($super, instanceID, widgetPath, sharedPath, sitePath, preferences, runningInApp) {
    if (instanceID != null) {
        ...
        NotificationCenter.addObserver(this, HeaderControls.prototype.p_prevPage, "PreviousPage", this.p_mediaGridID());
        NotificationCenter.addObserver(this, HeaderControls.prototype.p_nextPage, "NextPage", this.p_mediaGridID());
        NotificationCenter.addObserver(this, HeaderControls.prototype.p_setPage, "SetPage", this.p_mediaGridID());
        this.mRange = new IWPageRange(0, 5);
        this.p_updateRange();
    }
}, onload: function () {
    var defaults = {showBackToIndex: true, showSubscribe: true, showSlideshow: true, mediaIndex: false, entriesPerPage: 99, entryCount: 0};
    this.initializeDefaultPreferences(defaults);
    this.setPage(0);
    ...
    this.p_updatePaginationControls();
    this.p_updateSubscribe();
    this.p_updateSlideshow();
    if (this.p_mediaIndex()) {
...
}, p_canNavigateToPrev: function () {
    return(this.p_currentPage() > 0);
}, p_prevPage: function (notification) {
    if (this.p_canNavigateToPrev()) {
        this.setPage(this.p_currentPage() - 1);
    }
}, p_canNavigateToNext: function () {
    return(this.p_currentPage() < this.p_pageCount() - 1);
}, p_nextPage: function (notification) {
    if (this.p_canNavigateToNext()) {
        this.setPage(this.p_currentPage() + 1);
    }
}, p_setPage: function (notification) {
    var pageIndex = notification.userInfo().pageIndex;
    this.setPreferenceForKey(pageIndex, "x-currentPage");
    if (!this.runningInApp) {
        var entriesPerPage = this.p_entriesPerPage();
        var location = pageIndex * entriesPerPage;
        var length = Math.min(this.p_entryCount() - location, entriesPerPage);
        var userInfo = {"range": new IWRange(location, length)};
        NotificationCenter.postNotification(new IWNotification("RangeChanged", this.p_mediaGridID(), userInfo));
    }
}, p_showBackToIndex: function () {
    var show = this.preferenceForKey("showBackToIndex");
    (function () {
        return show !== undefined
    }).assert();
    return show;
}, p_showSubscribe: function () {
    var show = this.preferenceForKey("showSubscribe");
    (function () {
        return show !== undefined
    }).assert();
    return show;
    ...
    return mediaGridID;
}, p_setPagePreference: function () {
    var setPagePreference = null;
    if (this.preferences) {
...
}, p_updatePaginationControls: function () {
    var widgetDiv = this.div();
    var currentPage = this.p_currentPage();
    var controls = "";
    if (this.p_isPaginated()) {
        var canvasControlURLs = this.preferenceForKey("canvas controls");
        if (this.p_canNavigateToPrev()) {
            var leftArrowSrc = canvasControlURLs['canvas_arrow-left'];
            controls += "<a href='javascript:#{WIDGET_ID}.prevPage()'>";
            controls += imgMarkup(leftArrowSrc, '', '', '');
            controls += "</a> ";
        }
        else {
            var leftArrowSrc = canvasControlURLs['canvas_arrow-left-D'];
            controls += imgMarkup(leftArrowSrc, '', '', '') + " ";
        }
        ...
        if (this.p_canNavigateToNext()) {
            var rightArrowSrc = canvasControlURLs['canvas_arrow-right'];
            controls += "<a href='javascript:#{WIDGET_ID}.nextPage()'>";
            controls += imgMarkup(rightArrowSrc, '', '', '');
            controls += "</a>";
        }
        else {
            var rightArrowSrc = canvasControlURLs['canvas_arrow-right-D'];
            controls += imgMarkup(rightArrowSrc, '', '', '');
        }
    }
    ...
    widgetDiv.select(".paginated_only").invoke(this.p_isPaginated() ? 'show' : 'hide');
    widgetDiv.select(".non_paginated_only").invoke(this.p_isPaginated() ? 'hide' : 'show');
    if (this.runningInApp) {
...
}, p_setAnchorsUnderElementToHREF: function (element, href) {
    $(element).select('a').map(function (anchor) {
        anchor.href = href;
    });
...
}, p_updateBackToIndex: function () {
    var element = this.getElementById("back_to_index");
    this.p_showBackToIndex() ? element.show() : element.hide();
    if (!this.runningInApp) {
        this.p_setAnchorsUnderElementToHREF(element, this.p_indexURL());
    }
}, p_updateSubscribe: function () {
    this.div().select(".subscribe").invoke(this.p_showSubscribe() ? 'show' : 'hide');
    if (!this.runningInApp) {
        var feedURL = "javascript:" + this.instanceID + (this.p_mediaIndex() ? ".mediaIndexSubscribe()" : ".photocastSubscribe()")
        var self = this;
        this.div().select(".subscribe").each(function (element) {
            self.p_setAnchorsUnderElementToHREF(element, feedURL);
        });
    }
}, mediaIndexSubscribe: function () {
    window.location = this.p_feedURL();
}, photocastSubscribe: function () {
...
}, p_isPaginated: function () {
    return(this.p_entryCount() > this.p_entriesPerPage());
}, p_pageCount: function () {
    return Math.ceil(this.p_entryCount() / this.p_entriesPerPage());
}, p_updateRange: function () {
    var pageCount = this.p_pageCount();
    var currentPage = this.p_currentPage();
    if (currentPage >= pageCount) {
        currentPage = pageCount - 1;
        this.setPreferenceForKey(currentPage, "x-currentPage");
    }
