...
var Widget = Class.create({initialize: function (instanceID, widgetPath, sharedPath, sitePath, preferences, runningInApp) {
    if (instanceID) {
...
var JSONFeedRendererWidget = Class.create(Widget, {initialize: function ($super, instanceID, widgetPath, sharedPath, sitePath, preferences, runningInApp) {
    ...
    if (entryHasImage) {
        ...
        imgGroupDiv.setStyle({marginLeft: 0, marginTop: 0, marginRight: 0, marginBottom: 0});
    ...
    if (image.loaded()) {
        ...
        var croppingDivForImage_helper = function (loadedImage) {
            ...
            img.setStyle({width: px(scaled.width), height: px(scaled.height), marginLeft: px(-offset.x), marginTop: px(-offset.y), position: 'relative'});
            cropDiv.setStyle({width: px(cropped.width), height: px(cropped.height), overflow: "hidden", position: 'relative'});
...
var PrefMarkupWidget = Class.create(Widget, {initialize: function ($super, instanceID, widgetPath, sharedPath, sitePath, preferences, runningInApp) {
    if (instanceID) {
        $super(instanceID, widgetPath, sharedPath, sitePath, preferences, runningInApp);
    }
}, onload: function () {
...
var View = Class.create({initialize: function (widget, parentDiv) {
    ...
    if (!div) {
        div = new Element('div', {'id': this.m_widget.getInstanceId(this.m_divInstanceId)});
