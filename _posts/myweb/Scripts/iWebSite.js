...
Element.Methods = {visible: function (element) {
    ...
    if (typeof name == 'object')attributes = name; else attributes[name] = value === undefined ? true : value;
    ...
    if (oBlock) {
        if (elementHasCSSBGPNG(oBlock)) {
            ...
            var fixupIEPNGBG_helper = function (img) {
                ...
                wrappedContent.setStyle({position: "relative", zIndex: "1", left: 0, top: 0, background: "transparent"});
    ...
    if (windowsInternetExplorer) {
        ...
        if (oNode && (parseFloat(oNode.currentStyle.getAttribute('opacity')) < 1)) {
            ...
            if (oNode.tagName.toLowerCase() == 'img') {
                ...
                targetNode.setStyle({position: oNode.style.position, top: oNode.style.top, left: oNode.style.left, width: oNode.style.width, height: oNode.style.height, opacity: oNode.style.opacity, zIndex: oNode.style.zIndex});
                ...
                targetNode.setStyle({position: positioned ? positionStyleVal : "relative", styleFloat: floatStyleVal, clear: oNode.getStyle("clear"), width: px(extents.right - extents.left), height: px(extents.bottom - extents.top), opacity: oNode.style.opacity, zIndex: oNode.style.zIndex});
                ...
                oNode.setStyle({position: "absolute", styleFloat: "none", clear: "none", left: px(-extents.left), top: px(-extents.top), margin: 0, verticalAlign: 'baseline', display: 'block', opacity: ''});
...
var IWPageRange = Class.create(IWRange, {initialize: function ($super, location, length) {
...
var IWExtents = Class.create({initialize: function (left, top, right, bottom) {
    this.left = left;
    this.top = top;
    this.right = right;
    this.bottom = bottom;
...
var IWPadding = Class.create({initialize: function (left, top, right, bottom) {
    this.left = left;
    this.top = top;
    this.right = right;
    this.bottom = bottom;
}});
var IWNotificationCenter = Class.create({initialize: function () {
...
var IWNotification = Class.create({initialize: function (name, object, userInfo) {
    this.mName = name;
    this.mObject = object;
...
Object.extend(Array.prototype, {contains: function (value) {
