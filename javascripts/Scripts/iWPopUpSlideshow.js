...
function initSlideshow(imageStream, index, parameters) {
    ...
    if (checkBrowser()) {
        ...
        slideWidth = windowWidth - 200;
        slideHeight = windowHeight - 290;
        frameHeight = slideHeight + 50;
        frameWidth = slideWidth - 20;
        var frame = appendDiv(document.body, 'frame', frameWidth, frameHeight);
        var frameTop = Math.max(Math.round((windowHeight - frameHeight) / 2), 0);
        frame.style.marginTop = frameTop + 'px';
        var slideMatte = appendDiv(frame, 'matte', slideWidth, frameHeight);
        ...
        parameters.movieMode = kAutoplayMovie;
        slideshow = new Slideshow(slideMatte, photos, onchange, parameters);
        if (parameters.hasOwnProperty("transitionIndex")) {
            slideshow.setTransitionIndex(parameters.transitionIndex, 0);
        }
        initControls();
        var initThumbFade = function () {
            thumbnails.fadeHandler = null;
            thumbnails.fade(0.80, 0, 4000);
        };
        var initControlFade = function () {
            hud.fadeHandler = null;
            hud.fade(0.80, 0, 4000);
        };
        initThumbnailPicker(index != null ? index : 0, imageStream);
        thumbnails.fadeHandler = setTimeout(initThumbFade, 1000);
        hud.fadeHandler = setTimeout(initControlFade, 1000);
        slideshow.playHandler = setTimeout(beginPlay.bind(null, index), 0);
...
function beginPlay(index) {
    ...
    if (index != null && index > 0) {
        ...
        slideshow.showPhotoNumber(index, true);
    }
    else {
        slideshow.start();
    }
...
function initThumbnailPicker(startPoint, imageStream) {
    var thumbnailZone = appendDiv(document.body, 'thumbnailZone');
    var thumbnailPicker = appendDiv(thumbnailZone, 'thumbnailPicker');
    var thumbStrip = appendDiv(thumbnailPicker, 'thumbStrip');
    var pickerWidth = Math.min((imageStream.length * 89) + 2, frameWidth);
    thumbnailZone.style.width = windowWidth + "px";
    thumbnailZone.style.height = pickerHeight + 25 + "px";
    thumbnailZone.style.top = "10px";
    thumbnailPicker.style.width = pickerWidth + "px";
    thumbnailPicker.style.height = pickerHeight + "px";
    thumbnailPicker.style.left = Math.round((windowWidth - pickerWidth) / 2) + "px";
    thumbStrip.style.left = '0px';
    $(thumbnailPicker).setOpacity(thumbnailPicker, 0.8);
    addEvent(thumbnailZone, 'onmouseover', thumbnailFadeIn, false);
    addEvent(thumbnailZone, 'onmouseout', thumbnailFadeOut, false);
    var results = generateThumbs(imageStream);
    selectThumb(startPoint);
    initScroller(startPoint);
    thumbnails = new hoverControls('thumbnailPicker');
}
function initControls() {
    var controlZone = appendDiv(document.body, 'controlZone');
    ...
    var pauseButton;
    var frameTop = Math.max(Math.round((windowHeight - frameHeight) / 2), 0);
    controlZone.style.top = frameTop + frameHeight + "px";
    if ($('matte').style.marginLeft) {
        controls.style.left = Math.round((windowWidth - 177) / 2) + parseInt($('matte').style.marginLeft, 10) + "px";
    } else {
        controls.style.left = Math.round((windowWidth - 177) / 2) + parseInt($('matte').offsetLeft, 10) + "px";
    }
    if (browser.supportsPNGs) {
...
function selectThumb(index) {
    var thumbPicker = $('thumbStrip');
    var thumbs = thumbPicker.select('img');
    for (var i = 0; i < thumbs.length; ++i) {
        var thumb = thumbs[i];
        var className = (i == index) ? 'selectedthumb' : 'thumb';
        if (thumb.className != className) {
...
var thumbWidth = 69;
var thumbHeight = 69;
function thumbnailLoaded(thumbnail, index) {
    var thumbPicker = $('thumbStrip');
    var thumbPickerPlaceholder = $('thumb' + index);
    if (thumbPickerPlaceholder) {
        var naturalSize = thumbnail.naturalSize();
        var imageScale = thumbWidth / naturalSize.width < thumbHeight / naturalSize.height ? thumbWidth / naturalSize.width : thumbHeight / naturalSize.height;
        var scaledWidth = naturalSize.width * imageScale;
        var scaledHeight = naturalSize.height * imageScale;
        var horizontalMargin = px(Math.round((thumbWidth - scaledWidth) / 2) + 7);
        var verticalMargin = px(Math.round((thumbHeight - scaledHeight) / 2));
        thumbPickerPlaceholder.width = scaledWidth;
        thumbPickerPlaceholder.height = scaledHeight;
        thumbPickerPlaceholder.setStyle({margin: verticalMargin + " " + horizontalMargin});
        thumbPickerPlaceholder.src = thumbnail.sourceURL();
    }
    else {
...
function generateThumbs(imageStream) {
    var thumbPicker = $('thumbStrip');
    thumbPicker.style.width = (imageStream.length * 89) + "px";
    var markup = "";
    for (var i = 0, len = imageStream.length; i < len; ++i) {
    ...
    thumbPicker.innerHTML = markup;
    var index = 0;
    $A(imageStream).invoke('micro').each(function (thumbnail) {
        thumbnail.load(thumbnailLoaded.bind(null, thumbnail, index), index >= 10);
        index++;
    });
    return true;
}
function initScroller(startPoint) {
    ...
    dragTool = $(appendImage(dragTool, 'http://www.me.com/1/slideshow/images/dragger.gif', null, 24, 13));
    ...
    if (browser.isIE6x && browser.isWin) {
        scrollbar.style.top = parseInt(scrollbar.style.top, 10) - 2 + 'px';
        bar.style.height = parseInt(bar.style.height, 10) + 2 + 'px';
        scrollbar.style.width = parseInt(scrollbar.style.width, 10) + 2 + 'px';
    } else if (browser.isSafariJaguar) {
        bar.style.width = $('thumbnailPicker').getWidth() - 3 + 'px';
    }
}
...
function thumbnailFadeIn(evt) {
    try {
        ...
        if (checkMouseEnter($('thumbnailZone'), evt)) {
            if (!thumbnails.holdFade) {
                scroller.jumpTo(slideshow.currentPhotoNumber);
                slideshow.pause();
                thumbnails.fade(0.90, 0.90, 0);
            }
        }
    } catch (e) {
    }
...
function thumbnailFadeOut(evt) {
    try {
        evt = (evt) ? evt : ((window.event) ? window.event : "");
        if (checkMouseLeave($('thumbnailZone'), evt)) {
            if (!thumbnails.holdFade) {
                thumbnails.fade(0.30, 0, 2000);
...
function hudFadeIn(evt) {
    try {
        evt = (evt) ? evt : ((window.event) ? window.event : "");
        if (checkMouseEnter($('controlZone'), evt)) {
            hud.fade(0.80, 0.80, 0);
        }
    } catch (e) {
    }
}
function hudFadeOut(evt) {
    try {
        evt = (evt) ? evt : ((window.event) ? window.event : "");
        if (checkMouseLeave($('controlZone'), evt) && $(controls).getOpacity() > 0.05) {
            hud.fade(0.40, 0, 2000);
            if (slideshow.playHandler === null) {
                slideshow.resume();
            }
        }
...
function stop() {
    ...
    $('pausebutton').src = browser.supportsPNGs ? baseURL + 'images/pause.png' : baseURL + 'images/pause.gif';
    $('pausebutton').style.display = 'none';
    return false;
}
function pauseClick() {
    $('pausebutton').src = browser.supportsPNGs ? baseURL + 'images/pause_on.png' : baseURL + 'images/pause_on.gif';
    return false;
}
function restart() {
    slideshow.playHandler = null;
    $('pausebutton').style.display = '';
    $('playbutton').src = browser.supportsPNGs ? baseURL + 'images/play.png' : baseURL + 'images/play.gif';
    $('playbutton').style.display = 'none';
    if ($('controls').getOpacity() > 0.05) {
        hud.fade(0.90, 0, 5000);
    }
    slideshow.playHandler = setTimeout(function () {
        slideshow.resume();
    }, 1500);
}
function playClick() {
    $('playbutton').src = browser.supportsPNGs ? baseURL + 'images/play_on.png' : baseURL + 'images/play_on.gif';
    return false;
}
function forwardClick() {
    $('forwardbutton').src = browser.supportsPNGs ? baseURL + 'images/arrow_right_on.png' : baseURL + 'images/arrow_right_on.gif';
    return false;
}
function next() {
    $('forwardbutton').src = browser.supportsPNGs ? baseURL + 'images/arrow_right.png' : baseURL + 'images/arrow_right.gif';
    slideshow.advance();
    return false;
}
function backClick() {
    $('backbutton').src = browser.supportsPNGs ? baseURL + 'images/arrow_left_on.png' : baseURL + 'images/arrow_left_on.gif';
    return false;
}
function previous() {
    $('backbutton').src = browser.supportsPNGs ? baseURL + 'images/arrow_left.png' : baseURL + 'images/arrow_left.gif';
    slideshow.goBack();
    return false;
}
function arrowKeyDown(evt) {
    ...
    if ((slideshow.photos.length * thumbMatte) > (scroller.getContentClipW() - scroller.getContentL())) {
        scroller.jumpTo(slideshow.currentPhotoNumber, 'left');
    } else if ((slideshow.photos.length * thumbMatte) < -scroller.getContentL()) {
        scroller.jumpTo(slideshow.currentPhotoNumber, 'right');
    }
}
function arrowKeyUp(evt) {
    ...
    switch (keyCode) {
        ...
        case 35:
            slideshow.pause();
            slideshow.resetSlideshow(slideshow.photos.length - 1);
            if (slideshow.playHandler != -1) {
                slideshow.play();
            }
            break;
    }
    if ((slideshow.currentPhotoNumber * thumbMatte) > (scroller.getContentClipW() - scroller.getContentL())) {
        scroller.jumpTo(slideshow.currentPhotoNumber, 'left');
    } else if ((slideshow.currentPhotoNumber * thumbMatte) < -scroller.getContentL()) {
        scroller.jumpTo(slideshow.currentPhotoNumber, 'right');
    }
}
...
function BrowserDetectLite() {
    ...
    this.supportsPNGs = (!this.isIE || this.versionMajor >= 8);
}
...
function alphaRules() {
    ...
    if (browser.isWin && browser.isIE) {
        document.styleSheets[0].addRule("img", "filter: alpha(opacity=100)", 0);
    }
}
...
function hoverControls(divId) {
    ...
    var me = this;
    this.fade = function (start, end, ms) {
        start = $(this.divId).getOpacity();
        if (this.animation) {
            this.animation.stop();
        }
        if (this.fadeHandler) {
        ...
        this.animation.startOpacity = start;
        this.animation.endOpacity = end;
        this.animation.duration = ms;
...
function Scroller() {
    var scrollW = $('thumbnailPicker').getWidth();
    var speed = 4;
    var mouseY;
    var mouseX;
    var clickLeft = false;
    ...
    var eventLoader = function eventLoader() {
        scrollbarOffset = IWChildOffset($('scrollbar'), document.body);
        dragL = parseInt($('dragtool').style.left, 10);
        dragT = parseInt($('dragtool').style.top, 10);
        rulerT = parseInt($('ruler').style.top, 10);
        rulerL = parseInt($('ruler').style.left, 10);
        contentW = $('thumbStrip').getWidth();
        contentClipW = $('thumbnailPicker').getWidth();
        dragH = $('dragtool').getHeight();
        dragW = $('dragtool').getWidth();
    ...
    var up = function (e) {
        ...
        if (thumbnails && thumbnails.holdFade) {
            ...
            getMouse(e);
            var thumbZone = $('thumbnailZone');
            var zoneOffset = IWChildOffset(thumbZone, document.body);
            var zoneR = zoneOffset.x + thumbZone.getWidth();
            var zoneB = zoneOffset.y + thumbZone.getHeight();
            if ((mouseX + scrollbarOffset.x) > zoneOffset.x && (mouseX + scrollbarOffset.x) < zoneR && (mouseY + scrollbarOffset.y) > zoneOffset.y && (mouseY + scrollbarOffset.y) < zoneB) {
            } else {
                thumbnails.fade(0.30, 0, 2000);
                slideshow.resume();
            }
    ...
    var getL = function getL() {
        contentL = parseInt($('thumbStrip').style.left, 10);
    };
    var getMouse = function getMouse(e) {
        if (typeof scrollbarOffset == 'undefined' || typeof rightOffset == 'undefined') {
            me.resetPosition();
        }
        if (typeof event != 'undefined') {
            mouseY = event.clientY + document.body.scrollTop - scrollbarOffset.y;
            mouseX = event.clientX + document.body.scrollLeft - scrollbarOffset.x;
        } else {
            mouseY = e.pageY - scrollbarOffset.y;
            mouseX = e.pageX - scrollbarOffset.x;
        }
    };
    this.jumpTo = function jumpTo(slideCount, slidePosition) {
        if (contentW > contentClipW) {
            ...
            dragL = contentL * (scrollW - dragW) / (scrollW - contentW);
            if (dragL < 0) {
                dragL = 0;
            }
            return moveTo();
    ...
    var moveTo = function moveTo() {
        $('thumbStrip').style.left = contentL + "px";
        $('dragtool').style.left = dragL + "px";
        $('ruler').style.left = dragL + "px";
        return true;
    };
...
function isParent(container, containee) {
    var isParentBool = false;
    while (!isParentBool && containee) {
        isParentBool = container == containee;
